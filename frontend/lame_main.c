...
static char*
memstr(char* full_data, int full_data_len, char* substr)
{
...
static int
write_iTunSMPB(lame_global_flags * gf, FILE * outf, unsigned char *id3v2tag_origin, size_t  id3v2_size)
{
    char* p = memstr((char*)id3v2tag_origin, id3v2_size, "iTunSMPB");
    if (p != NULL) {
        lame_internal_flags const *gfc = gf->internal_flags;
        int delay = gfc->ov_enc.encoder_delay;
        int padding = gfc->ov_enc.encoder_padding;
        unsigned int total_samples = (unsigned int)gfc->ov_enc.frame_number * 1152;
        unsigned int real_samples = total_samples - (unsigned int)(delay + padding);
        int sample_rate = gf->samplerate_out;
        char buf[128];
        size_t owrite;

        sprintf(buf, " %08X %08X %08X 00000000%08X", 0, delay, padding, real_samples);
        memcpy(p + 9, buf, strlen(buf));
        fseek(outf, 0, SEEK_SET);
        owrite = fwrite(id3v2tag_origin, 1, id3v2_size, outf);
        if (owrite != id3v2_size) {
            error_printf("Error writing iTunSMPB \n");
            return 1;
        }

        MSGF(gfc, "iTunSMPB, delay: %d, padding: %d, real samples: %u, total samples: %u, real duration: %.6lf, total duration: %.6lf, sample rate: %d\n",
            delay, padding, real_samples, total_samples, (double)real_samples / sample_rate, (double)total_samples / sample_rate, sample_rate);
    }
    return 0;
}
...
lame_encoder_loop(lame_global_flags * gf, FILE * outf, int nogap, char *inPath, char *outPath)
{
    ...
    size_t  id3v2_size;
    unsigned char *id3v2tag_origin = NULL;

    id3tag_set_comment_latin1(gf, 0, "iTunSMPB", " 00000000 00000000 00000000 0000000000000000 00000000 00000000 00000000 00000000 00000000 00000000 00000000 00000000");
    ...
    if (id3v2tag_origin != NULL) {
        write_iTunSMPB(gf, outf, id3v2tag_origin, id3v2_size);
        free(id3v2tag_origin);
    }
